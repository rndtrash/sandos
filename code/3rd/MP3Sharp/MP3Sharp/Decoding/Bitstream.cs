...
namespace MP3Sharp.Decoding {
    ...
    public sealed class Bitstream {
        ...
        internal Bitstream(PushbackStream stream) {
            InitBlock();
            _SourceStream = stream ?? throw new Exception( "in stream is null, also why the fuck does s&box not have a NullReferenceException" );
            CloseFrame();
        }
        ...
        internal bool IsSyncMark(int headerstring, int syncmode, int word) {
            ...
            if (sync) {
                ...
                if (!sync) Log.Error("INVALID VERSION DETECTED");
            }
            return sync;
        }
